...
and finetuning policy with NPG.
Note that DAPG with no demo paths is simply RL (NPG in this case).
"""
...
# Finetune with NPG
print("========================================")
print("Finetuning with RL")
baseline = MLPBaseline(e.spec, reg_coef=1e-3, batch_size=64, epochs=2, learn_rate=1e-3)
# DAPG with no demo paths is RL finetuning
agent = DAPG(e, policy, baseline, demo_paths=None, normalized_step_size=0.1, seed=SEED, save_logs=True)

ts = timer.time()
train_agent(job_name='relocate_demo_init_rl_finetune',
            agent=agent,
            seed=SEED,
            niter=100,
            gamma=0.995,
